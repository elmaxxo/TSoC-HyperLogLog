...
/* Set a new rank for the register with the given index. */
static void
...
/*
 * Set a new rank for the register with the given index if it's greater
 * than the old rank.
 */
static void
hll_dense_set_register_rank_if_greater(struct hll *hll, uint32_t idx,
				       uint8_t new_rank)
{
...
hll_dense_add(struct hll *hll, uint64_t hash)
{
	...
	hll_dense_set_register_rank_if_greater(hll, idx, hash_rank);
}
...
static_assert(lengthof(pow_2_minus) == HLL_RANK_MAX + 1,
	      "Array must contain HLL_RANK_MAX + 1 values.");
...
hll_dense_add_pair(struct hll *hll, pair_t pair)
{
	...
	hll_dense_set_register_rank_if_greater(hll, idx, new_rank);
}
...
hll_sparse_grow_after_merge(struct hll *hll)
{
	...
	/* Point the buffer to the new last pair. */
...
hll_merge_dense_with_dense(struct hll *dst, const struct hll *src)
{
	...
	for (size_t idx = 0; idx < n_registers; ++idx) {
		...
		hll_dense_set_register_rank_if_greater(dst, idx, new_rank);
	}
}
